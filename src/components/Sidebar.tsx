  ...
  return (
    <div className="flex flex-col md:h-full glass-panel rounded-2xl md:overflow-hidden w-full md:w-[320px] lg:w-[365px] shrink-0 shadow-lg">
        ...
        <div className="flex-1 flex flex-col">
          ...
          {activeTab === 'comment' && (
            <div className="flex flex-col animate-in fade-in slide-in-from-bottom-2 duration-200">
              ...
              {(state.platform === 'tiktok' || state.platform === 'instagram') && (
                <div className="p-4.5 border-b border-[var(--panel-border)] flex flex-col gap-3">
                  {state.platform === 'tiktok' && (
                    <div>
                      <Label className="text-xs font-bold uppercase tracking-wider text-[var(--root-fg)] mb-2 block">{t('sidebar.tiktokTemplate')}</Label>
                  ...
                  {state.platform === 'instagram' && (
                    <div>
                      <Label className="text-xs font-bold uppercase tracking-wider text-[var(--root-fg)] mb-2 block">{t('sidebar.instagramTemplate')}</Label>
                      <div className="flex bg-[var(--root-bg)] border border-[var(--panel-border)] rounded-lg p-1">
                        <button
              ...
              {/* AVATAR */}
              <div className="p-4.5 border-b border-[var(--panel-border)] flex flex-col gap-3">
                <Label className="text-xs font-bold uppercase tracking-wider text-[var(--root-fg)]">{t('sidebar.avatar')}</Label>
              ...
              <div className="p-4.5 border-b border-[var(--panel-border)] flex flex-col gap-3">
                <div>
                  <Label className="text-xs font-bold uppercase tracking-wider text-[var(--root-fg)] mb-1.5">{t('sidebar.username')}</Label>
                  <Input 
                ...
                {state.platform === 'twitter' && (
                  <div>
                    <Label className="text-xs font-bold uppercase tracking-wider text-[var(--root-fg)] mb-1.5">{t('sidebar.handle')}</Label>
                    <Input 
                      value={state.handle} 
              ...
              <div className="p-4.5 border-b border-[var(--panel-border)] flex flex-col gap-2.5">
                <div className="flex justify-between items-end mb-1">
                  <Label className="mb-0 text-xs font-bold uppercase tracking-wider text-[var(--root-fg)]">{t('sidebar.commentText')}</Label>
          ...
          {activeTab === 'advanced' && (
            <div className="flex flex-col animate-in fade-in slide-in-from-bottom-2 duration-200">
              ...
              {/* ANIMATION FUTURE */}
              <div className="p-4.5 border-b border-[var(--panel-border)]">
                <Label className="text-[11px] font-bold uppercase tracking-wider text-[var(--text-muted)] mb-1.5">{t('sidebar.animation')}</Label>
              ...
              {/* CUSTOM CSS FUTURE */}
              <div className="p-4.5">
                <Label className="text-[11px] font-bold uppercase tracking-wider text-[var(--text-muted)] mb-1.5">{t('sidebar.customCss')}</Label>
                <div className="text-[10px] text-[var(--text-muted)] bg-[var(--root-bg)] border border-dashed border-[var(--panel-border)] rounded-xl p-3 leading-relaxed">
            ...
            <div className="flex flex-col p-4.5 animate-in fade-in slide-in-from-bottom-2 duration-200">
              ...
              {snapshots.length === 0 ? (
                <div className="flex flex-col items-center justify-center p-8 bg-[var(--root-bg)] border border-dashed border-[var(--panel-border)] rounded-2xl">
                  ...
                  <p className="text-xs text-[var(--text-muted)] text-center font-medium">{t('sidebar.gallery.empty')}</p>
                  <p className="text-[10px] text-[var(--text-muted)] text-center opacity-60 mt-1">{t('sidebar.gallery.emptyDesc')}</p>
                </div>
              ) : (
                      ...
                      <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-between p-2">
                        <div className="flex justify-between items-start">
                          ...
                          <button 
                            ...
                            title={t('sidebar.gallery.delete')}
                          >
      ...
      <div className="p-4 border-t border-[var(--panel-border)] bg-[var(--panel-bg-translucent)] backdrop-blur-md shrink-0 flex flex-col gap-3.5 z-10">
        ...
        <Accordion 
          title={t('sidebar.draft.count', { count: drafts.length })} 
